...
const APIURL = 'https://api.thegraph.com/subgraphs/name/omegachads/impressions-message-requests';
...
const MessageTile: FC<MessageTileProps> = ({ message, profile, currentProfile }) => {
  ...
  useEffect(() => {
    (async () => {
      const ads = await getAds(address);
      console.log('Ads: ', ads);
      setAds(ads);
    })();
  }, []);
      ...
      <div className="flex max-w-[60%]">
        ...
        >
          ...
          {address !== message.senderAddress  ? (
            <span>
              {' '}
              <Claim request={ads[ads.length - 1]} message={message} />{' '}
            </span>
          ) : (
            <span></span>
          )}
...
const MissingXmtpAuth: FC = () => (
  <Card as="aside" className="mb-2 mr-4 border-gray-400 !bg-gray-300 !bg-opacity-20 space-y-2.5 p-5">
    <div className="flex items-center space-x-2 font-bold">
      ...
      <p>This friend hasn't enabled DMs yet</p>
    </div>
...
const ConversationBeginningNotice: FC = () => (
  <div className="flex align-items-center justify-center mt-6 pb-4">
    <span className="text-gray-300 text-sm font-bold">Start of </span>
  </div>
);
